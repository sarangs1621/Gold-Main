...
class JobCardItem(BaseModel):
    ...
    making_charge_type: Optional[str] = None  # 'flat', 'per_gram', or 'per_inch' (MODULE 2)
    making_charge_value: Optional[float] = None  # Used for flat and per_gram
    # MODULE 2: Per-inch making charge fields
    length_in_inches: Optional[float] = None  # Required when making_charge_type='per_inch'
    rate_per_inch: Optional[float] = None  # Required when making_charge_type='per_inch'
...
class PurchaseItem(BaseModel):
    """
    MODULE 4: Individual item in a purchase - supports multiple items with different purities.
    
    CRITICAL VALUATION RULE:
    - entered_purity: Stored for reference only (as claimed by vendor)
    - calculated_amount: ALWAYS uses 916 purity via formula:
      amount = (weight_grams × 916) ÷ conversion_factor
    """
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    description: str  # Item description
    weight_grams: float  # 3 decimal precision - actual weight
    entered_purity: int  # Purity as entered/claimed (e.g., 999, 995, 916) - REFERENCE ONLY
    calculated_amount: float  # Auto-calculated using 22K (916) valuation - 2 decimal precision
...
class Purchase(BaseModel):
    ...
    - entered_purity: Stored for reference only, NOT used in valuation
    ...
    - If walk_in: vendor_party_id can be Optional, walk_in_name required
    - customer_id: Optional for walk-ins
    """
    model_config = ConfigDict(extra="ignore")
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    
    # MODULE 4: Vendor information (saved or walk-in)
    vendor_type: str = "saved"  # "saved" or "walk_in"
    vendor_party_id: Optional[str] = None  # Required for saved vendors, optional for walk-ins
    walk_in_name: Optional[str] = None  # Required for walk-in vendors
    walk_in_customer_id: Optional[str] = None  # Optional Oman Customer ID for walk-ins
    
    date: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    description: str
    
    # MODULE 4: Multiple items support
    items: List[PurchaseItem] = []  # List of purchase items (different purities allowed)
    
    # MODULE 4: Conversion factor (0.920 or 0.917)
    conversion_factor: float  # Required: 0.920 or 0.917 - used in 22K valuation formula
    
    # Backward compatibility: Single-item fields (deprecated but kept for old data)
    weight_grams: Optional[float] = None  # DEPRECATED: Use items[] instead
    entered_purity: Optional[int] = None  # DEPRECATED: Use items[] instead
    valuation_purity_fixed: int = 916  # ALWAYS 916 for stock calculations and accounting
    rate_per_gram: Optional[float] = None  # DEPRECATED: Calculation now uses conversion_factor
...
class ShopSettings(BaseModel):
    """Shop/Company settings for invoice printing and MODULE 4 purchase configuration"""
    model_config = ConfigDict(extra="ignore")
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    ...
    # MODULE 4: Purchase conversion factors (for 22K valuation formula)
    conversion_factors: List[float] = [0.920, 0.917]  # Available conversion factors for purchase valuation
    default_conversion_factor: float = 0.920  # Default conversion factor for new purchases
    updated_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
...
async def finalize_purchase(purchase_id: str, current_user: User = Depends(require_permission('purchases.finalize'))):
    ...
    for item in purchase.items:
        weight_decimal = Decimal(str(item.weight_grams)).quantize(Decimal('0.001'))
        movement_dict = {
            "id": str(uuid.uuid4()),
            "date": purchase.date,
            "created_at": datetime.now(timezone.utc),
...
async def get_returnable_invoices(
    ...
    - Are not deleted
    - Have at least some items NOT fully returned yet
    ...
    # Also filter out fully returned invoices
    formatted_invoices = []
    for inv in invoices:
        # Calculate total qty from invoice items
        invoice_items = inv.get('items', [])
        total_qty = sum(item.get('qty', 0) for item in invoice_items)
        
        # Check how much has been returned (finalized returns only)
        ...
        for ret in returns:
            ret_items = ret.get('items', [])
            returned_qty += sum(item.get('qty', 0) for item in ret_items)
        
        # Only include if there's still qty available to return
        if returned_qty < total_qty:
            ...
            formatted_invoices.append({
                ...
                "remaining_qty": total_qty - returned_qty
            })
...
async def add_payment_to_invoice(
    ...
    if payment_mode == "GOLD_EXCHANGE":
        ...
        # Use helper function to create transaction with balance tracking
        await create_transaction_with_balance(credit_transaction, account['account_type'])
...
async def export_invoices_pdf(
    ...
    """Export invoices report as PDF with ALL invoices (multi-page support)"""
    from reportlab.lib.pagesizes import A4, landscape
    from reportlab.lib.units import inch
    from reportlab.lib import colors
    from reportlab.platypus import SimpleDocTemplate, Table, TableStyle, Paragraph, Spacer
    ...
    summary = data['summary']
    summary_text = f"""<b>Summary:</b><br/>
    Total Amount: {summary['total_amount']:.3f} | Total Paid: {summary['total_paid']:.3f} | Total Balance: {summary['total_balance']:.3f} | Count: {data['count']}
    """
    elements.append(Paragraph(summary_text, styles['Normal']))
    elements.append(Spacer(1, 0.3 * inch))
    
    # Create table data - NO LIMIT, include ALL invoices
    table_data = [['Invoice #', 'Date', 'Customer', 'Type', 'Amount', 'Paid', 'Balance']]
    for inv in data['invoices']:  # FIXED: Removed [:25] limit - now includes ALL invoices
        inv_date = inv.get('date', '')
        if isinstance(inv_date, str):
...
async def export_sales_history_pdf(
    ...
    elements.append(Paragraph("Sales History Report", title_style))
    ...
    summary = data['summary']
    summary_text = f"""<b>Summary:</b><br/>
    Total Invoices: {summary['total_invoices']} | Total Weight: {summary['total_weight']:.3f} g | Total Sales: {summary['total_sales']:.2f} OMR
    """
    elements.append(Paragraph(summary_text, styles['Normal']))
    elements.append(Spacer(1, 0.3 * inch))
    ...
    for record in data['sales_records']:  # FIXED: Removed [:30] limit - now includes ALL records
        table_data.append([
            (record.get('invoice_id') or '')[:15],
            (record.get('customer_name') or '')[:25],
            (record.get('customer_phone') or '')[:15],
