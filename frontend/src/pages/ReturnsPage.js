...
const ReturnsPage = () => {
  ...
  const handleFinalizeReturn = async () => {
    ...
    try {
      ...
      // MODULE 6: Show inventory action required notice prominently
      const inventoryActionRequired = response.data?.details?.inventory_action_required;
      let message = '✅ Return finalized successfully!';
      
      if (inventoryActionRequired) {
    ...
    <div className="p-6">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-blue-500 p-4 mb-4">
        <div className="flex">
          ...
          <div className="ml-3">
            <p className="text-sm font-semibold text-blue-900 mb-1">📋 Returns Module - Key Features</p>
            <ul className="text-sm text-blue-700 space-y-1 list-disc list-inside">
              <li><strong>Partial Returns:</strong> Return any number of items from an invoice (not forced to return all)</li>
              <li><strong>Draft → Finalize:</strong> Create drafts first, add refund details later, then finalize</li>
              <li><strong>Audit Safety:</strong> Completed returns are locked and cannot be deleted</li>
              <li><strong>Manual Inventory:</strong> Finalized returns require manual inventory adjustment (no auto-stock changes)</li>
            </ul>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 py-4 space-y-4">
              ...
              {/* MODULE 6: Inventory Action Required Notice */}
              {selectedReturn.status === 'finalized' && selectedReturn.inventory_action_required && (
                <div className="bg-orange-50 border-l-4 border-orange-500 p-4 mb-4">
                  <div className="flex items-start">
                    ...
                    <div className="ml-3">
                      <h3 className="text-sm font-medium text-orange-800">Manual Inventory Adjustment Required</h3>
                      <div className="mt-2 text-sm text-orange-700">
                        <p>{selectedReturn.inventory_action_notes || 'This return requires manual inventory adjustment by an administrator.'}</p>
                        {!selectedReturn.inventory_action_completed && (
                          <p className="mt-1 font-semibold">⚠️ Pending manual action</p>
                        )}
                        {selectedReturn.inventory_action_completed && (
                          <p className="mt-1 text-green-700 font-semibold">✅ Inventory adjustment completed</p>
                        )}
                      </div>
                    </div>
                  </div>
